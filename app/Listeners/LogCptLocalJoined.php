...
class LogCptLocalJoined
{
    public function handle(CptLocalJoined $event): void
    {
        $trainee = $event->cpt->trainee;
        $course  = $event->cpt->course;

        ActivityLog::create([
            'action'      => 'cpt.local_joined',
            'model_type'  => $event->cpt::class,
            'model_id'    => $event->cpt->id,
            'description' => "{$event->local->name} signed up as local mentor for {$trainee->name}'s CPT in {$course->name}",
