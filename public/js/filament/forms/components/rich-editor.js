...
me.prototype = {
	...
	update: function (t, e, n) {
		var r = n && n != t ? this.remove(n) : this,
			...
			i = r.content.slice()
		return (
			o == -1 ? i.push(n || t, e) : ((i[o + 1] = e), n && (i[o] = n)), new me(i)
		)
	},
	remove: function (t) {
		var e = this.find(t)
...
var Ar = class t {
	...
	ae = class t {
		...
		static fromJSON(e, n) {
			...
			const o = S.fromJSON(e, n.content),
...
var Oi = class {
		constructor(e, n, r) {
			;(this.hasDefault = Object.hasOwn(r, "default")),
				...
				(this.validate =
					typeof r.validate == "string" ? mp(e, n, r.validate) : r.validate)
		}
		get isRequired() {
		...
		constructor(e) {
			...
			for (const o in this.marks) {
				const i = this.marks[o],
					s = i.spec.excludes
				i.excluded = s == null ? [i] : s == "" ? [] : ua(this, s.split(" "))
			}
...
var Ke = class t {
		...
		static schemaRules(e) {
			...
			for (const o in e.marks) {
				...
				i &&
					i.forEach((s) => {
						r((s = ha(s))), s.mark || s.ignore || s.clearMark || (s.mark = o)
					})
			}
			for (const o in e.nodes) {
				...
				i &&
					i.forEach((s) => {
						r((s = ha(s))), s.node || s.ignore || s.mark || (s.node = o)
					})
			}
			return n
		}
	...
	bp = { head: !0, noscript: !0, object: !0, script: !0, style: !0, title: !0 },
	Da = { ol: !0, ul: !0 },
	Ln = 1,
	Ri = 2,
	In = 4
...
var ln = class {
		...
		findWrapping(e) {
			if (!this.match) {
				if (!this.type) return []
				const n = this.type.contentMatch.fillBefore(S.from(e))
		...
		finish(e) {
			...
			return (
				!e && this.match && (n = n.append(this.match.fillBefore(S.empty, !0))),
		...
		inlineContext(e) {
			return this.type
				...
				: this.content.length
					...
					: e.parentNode &&
						!Object.hasOwn(Ra, e.parentNode.nodeName.toLowerCase())
		}
	},
	Nr = class {
		constructor(e, n, r) {
...
`,
		...
		addElement(e, n, r) {
			...
			let s = e.nodeName.toLowerCase(),
				l
			Object.hasOwn(Da, s) && this.parser.normalizeLists && wp(e)
			const a =
				(this.options.ruleFromNode && this.options.ruleFromNode(e)) ||
				(l = this.parser.matchTag(e, this, r))
			e: if (a ? a.ignore : Object.hasOwn(bp, s))
			...
			for (let l = 0; l < this.ranges.length; l += 3) {
				...
				if (e <= u) {
					...
					let p = e == (n < 0 ? a : u) ? null : Cp(l / 3, e - a),
						m = e == a ? Ha : e == u ? za : Or
					return (n < 0 ? e != a : e != u) && (m |= $a), new Hn(h, m, p)
				}
				o += d - c
			}
	...
	for (let a = 0; a < i.childCount; a++) {
		...
		if (!u) s.push(new ge(l, d, E.empty))
			...
			for (let f = 0; f < c.marks.length; f++)
				n.allowsMarkType(c.marks[f].type) || t.step(new lt(l, d, c.marks[f]))
			if (o && c.isText && n.whitespace != "pre") {
				...
				for (; (f = h.exec(c.text)); )
					p ||
						...
						s.push(new ge(l + f.index, l + f.index + f[0].length, p))
			}
		}
		l = d
	}
	if (!r.validEnd) {
		const a = r.fillBefore(S.empty, !0)
...
function Ep(t, e, n) {
	...
	for (let p = i, m = !1; p > n; p--)
		m || r.index(p) > 0 ? ((m = !0), (d = S.from(r.node(p).copy(d))), u++) : a--
	let f = S.empty,
		h = 0
	for (let p = i, m = !1; p > n; p--)
...
var Bi = class {
		...
		for (let n = 1; n <= 2; n++)
			for (let r = n == 1 ? e : this.unplaced.openStart; r >= 0; r--) {
				let o,
					i = null
				r
					? ((i = Ii(this.unplaced.content, r - 1).firstChild), (o = i.content))
				...
				for (let l = this.depth; l >= 0; l--) {
					...
					if (
						n == 1 &&
						(s
							? c.matchType(s.type) || (u = c.fillBefore(S.from(s), !1))
	...
	placeNodes({
		...
		this.unplaced = p
			? e == 0
				? E.empty
				: new E(Bn(s.content, e - 1, 1), e - 1, h < 0 ? s.openEnd : e - 1)
	...
	closeFrontierNode() {
		const n = this.frontier.pop().match.fillBefore(S.empty, !0)
...
function Bn(t, e, n) {
	return e == 0
		...
		: t.replaceChild(0, t.firstChild.copy(Bn(t.firstChild.content, e - 1, n)))
}
function zn(t, e, n) {
	return e == 0
		? t.append(n)
...
function ja(t, e, n) {
	...
	return (
		...
		e > 0 &&
			...
			n <= 0 &&
				(r = r.append(
					t.type.contentMatch.matchFragment(r).fillBefore(S.empty, !0),
				))),
		t.copy(r)
	)
}
	...
	P = class {
		...
		replace(e, n = E.empty) {
			...
			for (let l = 0; l < s.length; l++) {
				...
				e.replaceRange(d.map(a.pos), d.map(c.pos), l ? E.empty : n),
					l == 0 && Ga(e, i, (r ? r.isInline : o && o.isTextblock) ? -1 : 1)
			}
		}
		replaceWith(e, n) {
		...
		static findFrom(e, n, r = !1) {
			...
			for (let i = e.depth - 1; i >= 0; i--) {
				const s =
					n < 0
						? fn(e.node(0), e.node(i), e.before(i + 1), e.index(i), n, r)
						: fn(e.node(0), e.node(i), e.after(i + 1), e.index(i) + 1, n, r)
				if (s) return s
			}
			return null
		}
...
var R = class t extends P {
	...
	static create(e, n, r = n) {
		const o = e.resolve(n)
		return new t(o, r == n ? o : e.resolve(r))
	}
	static between(e, n, r) {
		const o = e.pos - n.pos
		if (((!r || o) && (r = o >= 0 ? 1 : -1), !n.parent.inlineContent)) {
			const i = P.findFrom(n, r, !0) || P.findFrom(n, -r, !0)
			if (i) n = i.$head
		...
		return (
			e.parent.inlineContent ||
				(o == 0
					? (e = n)
					: ((e = (P.findFrom(e, -r, !0) || P.findFrom(e, r, !0)).$anchor),
						e.pos < n.pos != o < 0 && (e = n))),
			new t(e, n)
		)
	}
...
var Vt = class {
	...
	Br = class t {
		...
		static create(e) {
			const n = new Wn(e.doc ? e.doc.type.schema : e.schema, e.plugins),
				r = new t(n)
		...
		reconfigure(e) {
			const n = new Wn(this.schema, e.plugins),
				r = n.fields,
				o = new t(n)
		...
		static fromJSON(e, n, r) {
			...
			const o = new Wn(e.schema, e.plugins),
				i = new t(o)
			return (
				o.fields.forEach((s) => {
			...
			for (let s = r.depth; ; s--) {
				...
				if (l && l.slice.size < l.to - l.from) {
					if (e) {
						...
						a.setSelection(
							pn(i, "end")
								? P.findFrom(a.doc.resolve(a.mapping.map(o.pos, -1)), -1)
...
function ic(t, e, n) {
	...
	if (!c || c.from != i || (c instanceof ge && c.slice.size >= a - i)) return !1
	if (n) {
		const d = t.tr.step(c)
...
var qi = (t, e, n) => {
		...
		const i = o.nodeAfter
		if (uc(t, o, e, 1)) return !0
		if (r.parent.content.size == 0 && (pn(i, "start") || L.isSelectable(i))) {
...
function Kp(t) {
	return (e, n) => {
		...
		if (
			(f || ((i[0] = l ? { type: l } : null), (f = Ae(d.doc, u, i.length, i))),
			!f)
		)
			return !1
...
function Gp(t, e, n) {
	...
	return !r || !o || !r.type.compatibleContent(o.type)
		...
		: !r.content.size && e.parent.canReplace(i - 1, i)
			...
			: !e.parent.canReplace(i, i + 1) || !(o.isTextblock || Oe(t.doc, e.pos))
				? !1
...
function uc(t, e, n, r) {
	...
	if (
		...
		if (n) {
			let h = e.pos + i.nodeSize,
				p = S.empty
			for (let y = s.length - 1; y >= 0; y--) p = S.from(s[y].create(null, p))
			p = S.from(o.copy(p))
			const m = t.tr.step(
					new ie(e.pos - 1, h, e.pos, h, new E(p, 1, 0), s.length, !0),
				),
	...
	const d = i.type.spec.isolating || (r > 0 && a) ? null : P.findFrom(e, 1),
...
function fc(t) {
	return (e, n) => {
		...
		return o.node(i).isTextblock
			? (n &&
					n(e.tr.setSelection(R.create(e.doc, t < 0 ? o.start(i) : o.end(i)))),
				!0)
			: !1
	}
}
...
function Zp(t, e, n, r) {
	...
	return (
		Oe(o.doc, a.pos) && a.nodeBefore.type == a.nodeAfter.type && o.join(a.pos),
		e(o.scrollIntoView()),
		!0
	)
}
	...
	return (e, n) => {
		...
		if (n) {
			const c = a.lastChild && a.lastChild.type == l.type,
				...
				f = i.start,
				h = i.end
			n(e.tr.step(new ie(f - (c ? 3 : 1), h, f, h, u, 1, !0)).scrollIntoView())
		}
		return !0
	}
}
var fe = (t) => {
	...
	Jt = (t, e, n, r) => n && (yc(t, e, n, r, -1) || yc(t, e, n, r, 1)),
...
var Jr = (t) =>
	t.focusNode && Jt(t.focusNode, t.focusOffset, t.anchorNode, t.anchorOffset)
function _t(t, e) {
...
var ps = !Ce && /Chrome\/(\d+)/.exec(Nt),
	...
	am = Qn ? +(/\bAppleWebKit\/(\d+)/.exec(navigator.userAgent) || [0, 0])[1] : 0
function cm(t) {
...
function rd(t, e, n) {
	const {
		...
		s = Qn || Ie
	if (r.nodeType == 3)
		if (s && (wm.test(r.nodeValue) || (n < 0 ? !o : o == r.nodeValue.length))) {
			const a = vt(dt(r, o, o), n)
			if (Ie && o && /\s/.test(r.nodeValue[o - 1]) && o < r.nodeValue.length) {
...
var Pe = 0,
	...
	Gt = class {
		...
		domFromPos(e, n) {
			...
			for (
				let i;
				r && !(i = this.children[r - 1]).size && i instanceof Hr && i.side >= 0;
				r--
			);
			if (n <= 0) {
				let i,
...
`
						...
						for (let m = h, g; m; m = m.parentNode) {
							if ((g = m.nextSibling)) {
								g.nodeName == "BR" &&
									(l = a = { node: g.parentNode, offset: fe(g) + 1 })
								break
							}
				...
				(c, d, u) => {
					c.spec.marks
						...
						: c.type.side >= 0 &&
							!u &&
							a.syncToMarks(
								d == this.node.childCount ? q.none : this.node.child(d).marks,
								r,
								e,
							),
						a.placeWidget(c, e, o)
...
function id(t, e, n) {
	...
	for (let i = 0; i < e.length; i++) {
		...
		if (s instanceof kn) {
			...
			id(s.contentDOM, s.children, n), (r = a ? a.nextSibling : t.firstChild)
		}
	}
	for (; r; ) (r = Tc(r)), (o = !0)
...
var bs = class {
			...
			if (a instanceof Et) {
				...
				if (!f && (u = this.recreateWrapper(a, e, n, r, o, s)))
					return (
						...
						u.contentDOM &&
							((u.dirty = Wt), u.updateChildren(o, s + 1), (u.dirty = Pe)),
						(this.changed = !0),
						this.index++,
						!0
					)
	...
	addTextblockHacks() {
		let e = this.top.children[this.index - 1],
			n = this.top
		for (; e instanceof kn; ) (n = e), (e = n.children[n.children.length - 1])
		;(!e ||
			!(e instanceof $r) ||
			/\n$/.test(e.node.text) ||
...
function ad(t) {
	return t.editable
		? t.hasFocus()
		: dd(t) && document.activeElement && document.activeElement.contains(t.dom)
}
function ft(t, e = !1) {
		...
		if (n.indexOf("s") > -1) {
			...
			if (t.endOfTextblock(e > 0 ? "forward" : "backward")) {
				...
				return i.isAtom || ((s = t.docView.descAt(l)) && !s.contentDOM)
					? L.isSelectable(i)
						? Mt(t, new L(e < 0 ? t.state.doc.resolve(o.pos - i.nodeSize) : o))
						: Qn
							? Mt(t, new R(t.state.doc.resolve(e < 0 ? l : l + i.nodeSize)))
							: !1
					: !1
			}
		} else return !1
...
function Im(t) {
	...
	let o,
		...
		s = !1
	for (
		Ie && n.nodeType == 1 && r < _r(n) && Kn(n.childNodes[r], -1) && (s = !0);
		;
	)
		if (r > 0) {
...
function $m(t, e) {
	...
	if (n == 37 || (Re && n == 66 && r == "c")) {
		const o = n == 37 ? (Ic(t, t.state.selection.from) == "ltr" ? -1 : 1) : -1
		return Dc(t, o, r) || gn(t, o)
	} else if (n == 39 || (Re && n == 70 && r == "c")) {
		const o = n == 39 ? (Ic(t, t.state.selection.from) == "ltr" ? 1 : -1) : 1
		return Dc(t, o, r) || gn(t, o)
	} else {
		if (n == 38 || (Re && n == 80 && r == "c")) return Pc(t, -1, r) || gn(t, -1)
		if (n == 40 || (Re && n == 78 && r == "c"))
...
function Ds(t, e) {
	...
	const s = t.someProp("clipboardSerializer") || it.fromSchema(t.state.schema),
		l = yd(),
...
`,
	...
	else if (((l = E.maxOpen(Vm(l.content, o), !0)), l.openStart || l.openEnd)) {
		let u = 0,
			f = 0
		for (
		...
		if (
			(t.forEach((l) => {
				...
				let a = o.findWrapping(l.type),
					c
				if (!a) return (s = null)
				if ((c = s.length && i.length && pd(a, i, l, s[s.length - 1], 0)))
					s[s.length - 1] = c
				else {
					s.length && (s[s.length - 1] = md(s[s.length - 1], i.length))
					const d = hd(l, a)
...
function pd(t, e, n, r, o) {
	if (o < t.length && o < e.length && t[o] == e[o]) {
		const i = pd(t, e, n, r.lastChild, o + 1)
...
function md(t, e) {
	if (e == 0) return t
	const n = t.content.replaceChild(t.childCount - 1, md(t.lastChild, e - 1)),
			...
			(l =
				e < 0
					? s
							...
							.fillBefore(l, i <= o)
							.append(l)
					: l.append(s.contentMatchAt(s.childCount).fillBefore(S.empty, !0))),
	...
	for (const e in ke) {
		...
		t.dom.addEventListener(
			...
			(t.input.eventHandlers[e] = (r) => {
				Gm(t, r) && !Is(t, r) && (t.editable || !(r.type in Se)) && n(t, r)
			}),
	...
	t.someProp("handleDOMEvents", (e) => {
		for (const n in e)
			t.input.eventHandlers[n] ||
				t.dom.addEventListener(n, (t.input.eventHandlers[n] = (r) => Is(t, r)))
	})
}
function Is(t, e) {
...
function Xm(t, e) {
	!Is(t, e) && ke[e.type] && (t.editable || !(e.type in Se)) && ke[e.type](t, e)
}
Se.keydown = (t, e) => {
	const n = e
	if (
...
Se.keypress = (t, e) => {
	const n = e
	if (wd(t, n) || !n.charCode || (n.ctrlKey && !n.altKey) || (Re && n.metaKey))
		return
...
var Ms = class {
	...
	up(e) {
		...
		let n = this.pos
		this.view.state.doc != this.startDoc && (n = this.view.posAtCoords(Gr(e))),
...
function sg(t) {
	...
	const n = rm(e.focusNode, e.focusOffset),
...
function Wr(t, e = !1) {
	if (!(ut && t.domObserver.flushingSoon >= 0)) {
		if (
			(t.domObserver.forceFlush(), kd(t), e || (t.docView && t.docView.dirty))
		) {
			const n = Os(t),
				r = t.state.selection
			return (
...
function dg(t, e) {
	...
	setTimeout(() => {
		t.focus(),
			...
			n ? Jn(t, r.value, null, o, e) : Jn(t, r.textContent, r.innerHTML, o, e)
	}, 50)
}
function Jn(t, e, n, r, o) {
	const i = fd(t, e, n, r, t.state.selection.$from)
...
var Ur = class t {
	...
	Ut = class t {
		...
		eq(e) {
			return (
				this == e ||
				(e instanceof t && Gn(this.attrs, e.attrs) && Gn(this.spec, e.spec))
			)
		}
		static is(e) {
			...
			e.forEach((l, a) => {
				...
				if ((d = Md(n, l, c))) {
					...
					o[i] == a
						...
						: o.splice(i, 0, a, a + l.nodeSize, qr(d, l, c + 1, Kt)),
						(i += 3)
				}
			})
			const s = vd(i ? Td(n) : n, -r)
			for (let l = 0; l < s.length; l++)
	...
	Kr = class t {
		...
		forChild(e, n) {
			...
			for (let o = 0; o < this.members.length; o++) {
				...
				i != ye && (i instanceof t ? (r = r.concat(i.members)) : r.push(i))
			}
			return t.from(r)
		}
		eq(e) {
...
var mg = {
	...
	Es = class {
		...
		start() {
			this.observer &&
				(this.observer.takeRecords(), this.observer.observe(this.view.dom, mg)),
				this.onCharData &&
			...
			if (Ie && a.length) {
				...
				if (d.length == 2) {
					...
					for (const f of d) {
						...
						h && h.nodeName == "LI" && (!u || wg(e, u) != h) && f.remove()
					}
				}
			} else if (
				(de || be) &&
		...
		registerMutation(e, n) {
			...
			if (e.type == "childList") {
				...
				const s = o && o.parentNode == e.target ? fe(o) + 1 : 0,
					...
					a =
						i && i.parentNode == e.target ? fe(i) : e.target.childNodes.length,
...
function Cg(t, e, n, r, o) {
	...
	if (((t.input.compositionPendingChanges = 0), e < 0)) {
		...
		if (T && !t.state.selection.eq(T)) {
			if (
				...
				return
			const O = t.state.tr.setSelection(T)
			k == "pointer"
				? O.setMeta("pointer", !0)
	...
	if (!p)
		if (
			...
			if (c.sel) {
				const k = Wc(t, t.state.doc, c.sel)
				if (k && !k.eq(t.state.selection)) {
					const T = t.state.tr.setSelection(k)
	...
	let x = p.start,
		...
		w = (k) => {
			const T =
				k ||
				t.state.tr.replace(x, v, c.doc.slice(p.start - c.from, p.endB - c.from))
			if (c.sel) {
				const O = Wc(t, T.doc, c.sel)
				O &&
					!(
						(de &&
							t.composing &&
	...
	if (b)
		if (m.pos == g.pos) {
			...
			const k = w(t.state.tr.delete(x, v)),
			...
			const k = w(t.state.tr)
			C.type == "add" ? k.addMark(x, v, C.mark) : k.removeMark(x, v, C.mark),
				t.dispatch(k)
		} else if (
			...
			t.someProp("handleTextInput", (O) => O(t, x, v, k, T)) || t.dispatch(T())
		} else t.dispatch(w())
	else t.dispatch(w())
}
function Wc(t, e, n) {
...
function Tg(t, e, n, r, o) {
	...
	if (s < i && t.size < e.size) {
		...
		;(i -= a),
			i && i < e.size && jc(e.textBetween(i - 1, i + 1)) && (i += a ? 1 : -1),
			(l = i + (l - s)),
			(s = i)
	} else if (l < i) {
		const a = r <= i && r >= l ? i - r : 0
		;(i -= a),
			i && i < t.size && jc(t.textBetween(i - 1, i + 1)) && (i += a ? 1 : -1),
			(s = i + (s - l)),
			(l = i)
	}
...
var Xn = class {
	constructor(e, n) {
		;(this._root = null),
			...
			(this.docView = Mc(this.state.doc, Uc(this), cs(this), this.dom, this)),
			(this.domObserver = new Es(this, (r, o, i, s) => Cg(this, r, o, i, s))),
	...
	update(e) {
		...
		const n = this._props
		;(this._props = e),
			e.plugins && (e.plugins.forEach(Gc), (this.directPlugins = e.plugins)),
	...
	updateStateInner(e, n) {
		...
		let o = this.state,
			i = !1,
			s = !1
		e.storedMarks && this.composing && (kd(this), (s = !0)), (this.state = e)
		...
		const f =
			d == "preserve" && s && this.dom.style.overflowAnchor == null && um(this)
		if (s) {
...
var se
for (Xr in ht) Object.hasOwn(Yr, Xr) || (Yr[Xr] = ht[Xr])
var Xr
function Ad(t) {
	var e =
...
function Hd(t, e, n = {}) {
	return t.find(
		(r) =>
			r.type === e &&
			ro(Object.fromEntries(Object.keys(n).map((o) => [o, r.attrs[o]])), n),
	)
}
function Nd(t, e, n = {}) {
	return !!Hd(t, e, n)
...
var Yg =
		...
		({ editor: n, view: r, tr: o, dispatch: i }) => {
			...
			const l = $d(o.doc, t) || n.state.selection,
		...
		({ tr: r, dispatch: o, editor: i }) => {
			...
			if (o) {
				...
				if (
					...
					try {
						er(e, i.schema, { parseOptions: c, errorOnInvalidContent: !0 })
					} catch (g) {
						a(g)
					}
				try {
				...
				if (f) {
					...
					const g = r.doc.resolve(d),
						...
						v = y.content.size > 0
					b && x && v && (d = Math.max(0, d - 1)), r.replaceWith(d, u, m)
				}
			...
			return (
				n.selection.ranges.forEach((c) => {
					r.doc.nodesBetween(c.$from.pos, c.$to.pos, (d, u) => {
						i &&
							i === d.type &&
							((a = !0), o && n.setNodeMarkup(u, void 0, Od(d.attrs, e))),
							s &&
								d.marks.length &&
								d.marks.forEach((f) => {
									s === f.type &&
										((a = !0),
										o && n.addMark(u, u + d.nodeSize, s.create(Od(f.attrs, e))))
								})
					})
				}),
				a
...
function H(t, e, n) {
	return t.config[e] === void 0 && t.parent
		...
		: typeof t.config[e] == "function"
			? t.config[e].bind({ ...n, parent: t.parent ? H(t.parent, e, n) : null })
			: t.config[e]
}
function Gs(t) {
	return t
		...
		l = Object.fromEntries(
			o.map((c) => {
				const d = r.filter((y) => y.type === c.name),
					...
					f = t.reduce((y, b) => {
						const x = H(b, "extendNodeSchema", u)
						return { ...y, ...(x ? x(c) : {}) }
					}, {}),
					h = Dd({
						...f,
						...
						linebreakReplacement: J(H(c, "linebreakReplacement", u)),
				...
				const m = H(c, "renderHTML", u)
				m && (h.toDOM = (y) => m({ node: y, HTMLAttributes: oo(y, d) }))
				const g = H(c, "renderText", u)
		...
		a = Object.fromEntries(
			i.map((c) => {
				const d = r.filter((g) => g.type === c.name),
					...
					f = t.reduce((g, y) => {
						const b = H(y, "extendMarkSchema", u)
						return { ...g, ...(b ? b(c) : {}) }
					}, {}),
					h = Dd({
						...f,
				...
				const m = H(c, "renderHTML", u)
				return (
					m && (h.toDOM = (g) => m({ mark: g, HTMLAttributes: oo(g, d) })),
					[c.name, h]
				)
			}),
		)
		...
		(o.forEach(({ $from: u, $to: f }) => {
			...
			t.doc.nodesBetween(h, p, (m, g) => {
				...
				const y = Math.max(h, g),
					...
					x = b - y
				;(s += x), l.push(...m.marks.map((v) => ({ mark: v, from: y, to: b })))
			})
		}),
		s === 0)
	)
...
function nr(t, { checkChildren: e = !0, ignoreWhitespace: n = !1 } = {}) {
	...
	if (e) {
		...
		return (
			t.content.forEach((i) => {
				o !== !1 &&
					(nr(i, { ignoreWhitespace: n, checkChildren: e }) || (o = !1))
			}),
			o
		)
	}
...
function Bd(t, e) {
	const n =
		t.storedMarks || (t.selection.$to.parentOffset && t.selection.$from.marks())
	if (n) {
...
var Uy =
		...
		({ tr: e, state: n, dispatch: r, editor: o }) => {
			...
			if (
				...
				t && Bd(n, o.extensionManager.splittableMarks), e.scrollIntoView()
			}
			return p
		},
	Ky =
		...
		({ tr: n, state: r, dispatch: o, editor: i }) => {
					...
					const x =
							a.indexAfter(-1) < a.node(-2).childCount
								...
								: a.indexAfter(-2) < a.node(-3).childCount
									? 2
									: 3,
						v = { ...no(f, a.node().type.name, a.node().attrs), ...e },
						w =
			...
			if (o) {
				...
				const w = v.filter((C) => x.includes(C.type.name))
				n.ensureMarks(w)
			}
			return !0
		},
	...
	qy =
		...
		({
			...
			if (!x) return !1
			const w = Xe((C) => Ld(C.type.name, u))(m)
			if (x.depth >= 1 && w && x.depth - w.depth <= 1) {
				if (w.node.type === h) return c.liftListItem(p)
				if (Ld(w.node.type.name, u) && h.validContent(w.node.content) && l)
					return a()
								...
								y < d &&
									...
									y >= d &&
										...
										s &&
											...
											g.marks.forEach((b) => {
												if (s === b.type && ((a = !0), o)) {
													const x = Math.max(y, d),
														v = Math.min(y + g.nodeSize, u)
													n.addMark(x, v, s.create({ ...b.attrs, ...e }))
												}
											}))
							}),
						h &&
	...
	return (
		s.forEach((f) => {
			...
			const p = a.state.tr,
				...
				{ commands: y, chain: b, can: x } = new so({ editor: n, state: m })
			f.handler({
				state: m,
				range: g,
				match: h,
		...
		r = new B({
				...
				apply(o, i, s) {
					...
					return (
						!!a &&
							setTimeout(() => {
								let { text: d } = a
								typeof d == "string" ? (d = d) : (d = Xs(S.from(d), s.schema))
								const { from: u } = a,
									f = u + d.length
								Zr({ editor: e, from: u, to: f, text: d, rules: n, plugin: r })
							}),
			...
			props: {
				handleTextInput(o, i, s, l) {
					return Zr({ editor: e, from: i, to: s, text: l, rules: n, plugin: r })
				},
				handleDOMEvents: {
					compositionend: (o) => (
						setTimeout(() => {
...
var tl = class {
		...
		get storage() {
			return {
				...(J(
					H(this, "addStorage", { name: this.name, options: this.options }),
				) || {}),
			}
		}
		configure(t = {}) {
		...
		(d) =>
			new B({
				...
				appendTransaction: (u, f, h) => {
					...
					if (b) {
						let { text: w } = y
						typeof w == "string" ? (w = w) : (w = Xs(S.from(w), h.schema))
						const { from: C } = y,
							k = C + w.length,
							T = f0(w)
						return a({ rule: d, state: h, from: C, to: { b: k }, pasteEvt: T })
					}
					const x = f.doc.content.findDiffStart(h.doc.content),
						v = f.doc.content.findDiffEnd(h.doc.content)
					if (!(!a0(x) || !v || x === v.b))
						return a({ rule: d, state: h, from: x, to: v, pasteEvt: s })
				},
			}),
	)
}
var ho = class {
	...
	get plugins() {
		...
		return Ys([...this.extensions].reverse()).flatMap((r) => {
			...
			const a = Ed(l)
			i.push(a)
			const c = H(r, "addInputRules", o)
			if (Pd(r, t.options.enableInputRules) && c) {
				const f = c()
				if (f && f.length) {
					const h = s0({ editor: t, rules: f }),
						p = Array.isArray(h) ? h : [h]
					i.push(...p)
				}
			}
			const d = H(r, "addPasteRules", o)
			if (Pd(r, t.options.enablePasteRules) && d) {
				const f = d()
				if (f && f.length) {
					const h = h0({ editor: t, rules: f })
					i.push(...h)
				}
			}
			const u = H(r, "addProseMirrorPlugins", o)
			if (u) {
				const f = u()
				i.push(...f)
			}
...
var j = class ru extends tl {
				...
				new B({
					...
					appendTransaction: (t, e, n) => {
						...
						const f = n.tr,
							...
							{ commands: p } = new so({ editor: this.editor, state: h })
	...
	pu = j.create({
		...
		addGlobalAttributes() {
			...
			const { nodeExtensions: t } = Cn(this.extensions)
			return [
				{
...
var mu = class extends o0 {
	constructor(t = {}) {
		super(),
			...
			(this.utils = { getUpdatedPosition: Ly, createMappablePosition: By }),
			this.setOptions(t),
	...
	createDoc() {
		let t
		try {
			t = Vs(this.options.content, this.schema, this.options.parseOptions, {
				...
				for (k.lastIndex = 0; ; ) {
					...
					if (!((p = O[2]) != null && p.endsWith(":::"))) {
						if (z) v += 1
										...
										_ = h.blockTokens(V),
											_.forEach((D) => {
												D.text &&
													...
													(D.tokens = h.inlineTokens(D.text))
											});
										_.length > 0;
									) {
							...
							return { type: e, raw: U, attributes: x, content: C, tokens: _ }
						}
					}
				}
			},
...
`
		...
		for (d += 1; d < l.length; ) {
			...
			if (w.trim() === "") {
				...
				if (k === -1) break
				if (
					(((o = (r = l[d + 1 + k].match(/^(\s*)/)) == null ? void 0 : r[1]) ==
					null
						? void 0
						: o.length) || 0) > m
				) {
	...
	updateOverlay() {
		...
		if (n) {
			const u = e.nodeBefore,
				f = e.nodeAfter
			if (u || f) {
				const h = this.editorView.nodeDOM(this.cursorPos - (u ? u.nodeSize : 0))
				if (h) {
...
function go(t, e) {
	...
	return (r, o, i) => {
		...
		const c = le.findGapCursorFrom(l, e, a)
...
he.from = (e) => (e instanceof he ? e : e && e.length ? new Cu(e) : he.empty)
var Cu = ((t) => {
	function e(r) {
					...
					(u, f) => {
						...
						if (o) {
							...
							h &&
								...
								c.push(new Qe(p, void 0, void 0, c.length + d.length))),
								i--,
								p && o.appendMap(p, i)
...
function L0(t, e, n, r) {
	...
	if (
		...
		return new Ze(
			t.done.addTransform(n, a ? e.selection.getBookmark() : void 0, r, bo(e)),
			Qt.empty,
			c,
			n.time,
			l ?? t.prevComposition,
		...
		new B({
			...
			props: {
				handleDOMEvents: {
					beforeinput(e, n) {
						const r = n.inputType,
							o = r == "historyUndo" ? cl : r == "historyRedo" ? dl : null
						return !o || !e.editable
							? !1
...
var w1 = j.create({
	...
	Nu = j.create({
		...
		extendNodeSchema(t) {
			...
			return {
				allowGapCursor: (e = J(H(t, "allowGapCursor", n))) != null ? e : null,
			}
		},
	}),
	ul = j.create({
							...
							return (
								t.descendants((s, l) => {
									...
									if ((a || !this.options.showOnlyCurrent) && c) {
										...
										const u = ee.node(l, l + s.nodeSize, {
											class: d.join(" "),
											"data-placeholder":
...
`),
									...
									if (x === 0) return !0
									let v = d
									for (let C = 0; C < p; C += 1) v += h[C].length + 1
									return (
										l.delete(v, v + x),
										c - v <= x && l.setSelection(R.create(l.doc, v)),
										!0
									)
								})
							: t.commands.command(({ tr: l }) => {
...
`)
				...
				ArrowDown: ({ editor: t }) => {
					...
					return l === void 0
						...
						: r.nodeAt(l)
							? t.commands.command(
									({ tr: c }) => (c.setSelection(P.near(r.resolve(l))), !0),
								)
							: t.commands.exitCode()
				},
			}
...
`,
							...
							return (
								...
								s.selection.$from.parent.type !== this.type &&
									s.setSelection(
										R.near(s.doc.resolve(Math.max(0, s.selection.from - 2))),
									),
								s.setMeta("paste", !0),
								t.dispatch(s),
								!0
			...
			if (e.attrs.preview) {
				...
				;(c.className = "fi-fo-rich-editor-custom-block-preview fi-not-prose"),
					(c.innerHTML = new TextDecoder().decode(
						Uint8Array.from(atob(e.attrs.preview), (d) => d.charCodeAt(0)),
					)),
					s.appendChild(c)
			}
			return { dom: s }
...
var xo = (t, e) => e.view.domAtPos(t).node.offsetParent !== null,
	...
	Bu = (t, e) => {
		...
		if (!d) return !1
		const u = vn(d.node, (y) => y.type === i.nodes.detailsContent)
			...
			return ({ editor: t, getPos: e, node: n, HTMLAttributes: r }) => {
				...
				const a = (c) => {
					...
					const d = new Event("toggleDetailsContent"),
						u = l.querySelector(':scope > div[data-type="detailsContent"]')
					u?.dispatchEvent(d)
				}
				return (
							...
							t.chain()
								.command(({ tr: u }) => {
									...
									return h?.type !== this.type
										? !1
										: (u.setNodeMarkup(f, void 0, { open: !h.attrs.open }), !0)
								})
			...
			return {
				...
				unsetDetails:
					...
					({ state: t, chain: e }) => {
						...
						if (!o) return !1
						const i = vn(o.node, (y) => y.type === r.nodes.detailsSummary),
							s = vn(o.node, (y) => y.type === r.nodes.detailsContent)
		...
		addKeyboardShortcuts() {
			return {
				Enter: ({ editor: t }) => {
					...
					const v = x.createAndFill()
					if (!v) return !1
					const { tr: w } = e,
						C = o.after(-2)
					w.replaceWith(C, C, v)
					const k = w.doc.resolve(C),
						T = P.near(k, 1)
					w.setSelection(T)
					const O = m,
						A = m + p.nodeSize
									...
									if (l) {
										...
										if (a.nodeAfter)
											a.nodeAfter.isTextblock
												? s.setSelection(R.create(s.doc, a.pos + 1))
												: a.nodeAfter.isBlock
													? s.setSelection(L.create(s.doc, a.pos))
													: s.setSelection(R.create(s.doc, a.pos))
										else {
											const d =
												...
												u = d?.create()
											u &&
												(s.insert(c, u), s.setSelection(R.create(s.doc, c + 1)))
										}
										s.scrollIntoView()
									}
									return !0
		...
		if (
			...
			if (r)
				if (s.t && typeof s.t == "string") {
					const a = Object.assign(bb(s.t, r), n)
					Zt(i, a, r)
				} else n && Zt(i, n, r)
			s.t = i
		}
...
function Ot(t, e, n, r, o) {
	...
	for (let l = 0; l < s - 1; l++) {
		const a = e[l]
		t.j[a] ? (i = t.j[a]) : ((i = new Me(r)), (i.jr = o.slice()), (t.j[a] = i)),
			(t = i)
	}
		...
		appendTransaction: (e, n, r) => {
			...
			if (
				(Zs(l).forEach(({ newRange: c }) => {
					...
					if (u && f) {
						const h = f.split(Nb).filter(Boolean)
						if (h.length <= 0) return !1
						const p = h[h.length - 1],
							m = u.pos + f.lastIndexOf(p)
						if (!p) return !1
						const g = Zo(p).map((y) => y.toObject(t.defaultProtocol))
						if (!Db(g)) return !1
						g.filter((y) => y.isLink)
							...
							.forEach((y) => {
								co(y.from, y.to, r.doc).some((b) => b.mark.type === t.type) ||
		...
		props: {
			handleClick: (e, n, r) => {
				...
				if (
					...
					if (!l) return s
					const a = Qs(e.state, t.type.name),
					...
					getAttrs: (t) => {
						...
						return !e ||
							!this.options.isAllowedUri(e, {
								defaultValidate: (n) => !!en(n, this.options.protocols),
					...
					({ commands: t, chain: e }) =>
						this.options.keepAttributes
							? e()
									...
									.updateAttributes($b, this.editor.getAttributes(pf))
									.run()
							: t.toggleList(
									this.name,
									this.options.itemTypeName,
			...
			if (t.tokens && t.tokens.length > 0)
				if (t.tokens.some((o) => o.type === "paragraph"))
					...
					if (o && o.type === "text" && o.tokens && o.tokens.length > 0) {
						if (
							((n = [{ type: "paragraph", content: e.parseInline(o.tokens) }]),
							t.tokens.length > 1)
						) {
					...
					return (
						this.options.listTypes.forEach(
							({ itemName: n, wrapperNames: r }) => {
								t.state.schema.nodes[n] !== void 0 && Bl(t, n, r) && (e = !0)
							},
						),
						e
					)
					...
					({ commands: t, chain: e }) =>
						this.options.keepAttributes
							? e()
									...
									.updateAttributes(Kb, this.editor.getAttributes(yf))
									.run()
							: t.toggleList(
									this.name,
									this.options.itemTypeName,
			...
			return ({ node: t, HTMLAttributes: e, getPos: n, editor: r }) => {
				const o = document.createElement("li"),
					...
					c = (d) => {
						...
						l.ariaLabel =
							((f =
								(u = this.options.a11y) == null ? void 0 : u.checkboxLabel) ==
							null
								? void 0
						...
						r.isEditable &&
							...
							r
								...
								.command(({ tr: f }) => {
									...
									return (
										f.setNodeMarkup(h, void 0, { ...p?.attrs, checked: u }), !0
									)
								})
								.run(),
							!r.isEditable &&
...
`,
		...
		markdownTokenizer: {
			...
			start(t) {
				var e
				const n =
					(e = t.match(/^\s*[-+*]\s+\[([ xX])\]\s+/)) == null ? void 0 : e.index
				return n !== void 0 ? n : -1
			},
			tokenize(t, e, n) {
				const r = (i) => {
							...
							r().upload(`componentFileAttachments.${l}.${y}`, m, () => {
								c(y).then((b) => {
									b &&
										...
										g === f.length - 1 && ti(d, "form-processing-finished"))
								})
							})
						}),
						!0
						...
						: f.length
							? (u.preventDefault(),
								...
								f.forEach((p, m) => {
									...
									const g = ("10000000-1000-4000-8000" + -1e11).replace(
										/[018]/g,
										(y) =>
											(
												y ^
									...
									r().upload(`componentFileAttachments.${l}.${g}`, p, () => {
										c(g).then((y) => {
											y &&
												...
												m === f.length - 1 && ti(d, "form-processing-finished"))
										})
									})
								}),
								!0)
...
function Yb(t) {
	...
	return (
		...
		x < l.pos && v >= l.pos
			? { range: { from: x, to: v }, query: g[0].slice(n.length), text: g[0] }
			: null
	)
}
...
function Zb({
	...
	function v(C, k) {
		var T
		try {
			const A = t.getState(C.state),
				z = A?.decorationId
					? C.dom.querySelector(`[data-decoration-id="${A.decorationId}"]`)
					: null,
				V = {
					editor: e,
					...
					items: [],
					command: (U) =>
						u({ editor: e, range: A?.range || { from: 0, to: 0 }, props: U }),
					decorationNode: z,
					clientRect: x(C, z),
				}
			;(T = y?.onExit) == null || T.call(y, V)
		} catch {}
		const O = C.state.tr.setMeta(k, { exit: !0 })
		C.dispatch(O)
	}
	const w = new B({
		key: t,
		view() {
			return {
				update: async (C, k) => {
					var T, O, A, z, V, U, _
					const D = (T = this.key) == null ? void 0 : T.getState(k),
						K = (O = this.key) == null ? void 0 : O.getState(C.state),
						I = D.active && K.active && D.range.from !== K.range.from,
						Y = !D.active && K.active,
					...
					;(g = {
						...
						text: sn.text,
						items: [],
						command: (np) => u({ editor: e, range: sn.range, props: np }),
						decorationNode: la,
						clientRect: x(C, la),
					}),
						Rn && ((A = y?.onBeforeStart) == null || A.call(y, g)),
						Sr && ((z = y?.onBeforeUpdate) == null || z.call(y, g)),
						(Sr || Rn) && (g.items = await f({ editor: e, query: sn.query })),
		...
		state: {
			...
			apply(C, k, T, O) {
				...
				if (((D.composing = z), A && (U || e.view.composing))) {
					...
					I && p({ editor: e, state: O, range: I.range, isActive: k.active })
						? ((D.active = !0),
							(D.decorationId = k.decorationId ? k.decorationId : Y),
							(D.range = I.range),
							(D.query = I.query),
							(D.text = I.text))
						: (D.active = !1)
		...
		props: {
			handleKeyDown(C, k) {
				var T, O, A, z
				const { active: V, range: U } = w.getState(C.state)
				if (!V) return !1
				if (k.key === "Escape" || k.key === "Esc") {
					const D = w.getState(C.state),
					...
					if (
						((O = y?.onKeyDown) == null
							? void 0
							: O.call(y, { view: C, event: k, range: D.range })) ||
						!1
					)
						return !0
					const oe = {
						...
						text: D.text,
						items: [],
						command: (Pt) => u({ editor: e, range: D.range, props: Pt }),
						decorationNode: I,
			...
			decorations(C) {
				const { active: k, range: T, decorationId: O, query: A } = w.getState(C)
				if (!k) return null
				const z = !A?.length,
					V = [a]
				return (
...
var Af = Zb
var ew = ({ editor: t, overrideSuggestionOptions: e, extensionName: n }) => {
		const r = new $()
		return {
			editor: t,
			char: "{{",
...
function lw(t) {
	...
	for (let c = 0, d = 0; c < n; c++) {
		...
		for (let p = 0; ; p++) {
			...
			for (let x = 0; x < y; x++) {
				if (x + c >= n) {
					;(i || (i = [])).push({ type: "overlong_rowspan", pos: d, n: y - x })
					break
				}
				const v = o + x * e
				for (let w = 0; w < g; w++) {
...
var G = class wt extends P {
	...
	replace(e, n = E.empty) {
		...
		const i = P.findFrom(e.doc.resolve(e.mapping.slice(r).map(this.to)), -1)
		i && e.setSelection(i)
	}
	replaceWith(e, n) {
...
function Xl(t, e) {
	...
	return e ? e.doc != t.doc && qf(e.doc, t.doc, 0, r) : t.doc.descendants(r), n
}
function kw(t, e, n, r) {
	const o = re.get(e)
	...
	for (let a = 0, c = n + 1; a < o.height; a++) {
		...
		if (f > 0) {
			...
			const p = []
			for (let g = 0; g < f; g++) {
				const y = we(t.schema)[h].createAndFill()
				y && p.push(y)
			}
...
function Jf(t, { map: e, tableStart: n, table: r }, o) {
	...
	for (let s = 0; s < e.height; s++) {
		...
		if (o > 0 && o < e.width && e.map[l - 1] == e.map[l]) {
			...
			const a =
					i == null ? we(r.type.schema).cell : r.nodeAt(e.map[l + i]).type,
...
function Cw(t, e, n) {
	var r
	const o = we(e.type.schema).header_cell
	...
	if (e) {
		...
		if (
			(i.setNodeMarkup(a + r.tableStart, null, {
				...Uf(c.attrs, c.attrs.colspan, r.right - r.left - c.attrs.colspan),
...
function Ow(t, e) {
	...
	for (let o = 0; o < n.length; o++)
		if ((o >= e.length && e.push(S.empty), n[o] < r)) {
			const i = we(t).cell.createAndFill(),
				s = []
...
function Dw(t, e, n, r, o, i, s) {
	...
	if (i > e.height) {
		...
		const f = a.row.create(null, S.from(u)),
			h = []
...
function Lw(t, e, n) {
	...
	if (o instanceof G) {
		...
		const i = li(t.state),
			s = i.start(-1)
		return (
			$f(t.state, t.dispatch, s, re.get(i.node(-1)).findCell(i.pos - s), r), !0
		)
	} else return !1
}
function Bw(t, e) {
	...
	function s() {
		t.root.removeEventListener("mouseup", s),
			...
			It.getState(t.state) != null && t.dispatch(t.state.tr.setMeta(It, -1))
	}
	function l(a) {
		let c = a,
	...
	const i = new B({
		...
		state: {
			init(s, l) {
				...
				return r && c && (c[d] = (u, f) => new r(u, n, f)), new Hw(-1, !1)
			},
			apply(s, l) {
				return l.apply(s)
			},
...
var Hw = class si {
	...
	apply(e) {
		...
		if (r && r.setDragging !== void 0)
			return new si(this.activeHandle, r.setDragging)
		if (this.activeHandle > -1 && e.docChanged) {
			let o = e.mapping.map(this.activeHandle, -1)
			return Kl(e.doc.resolve(o)) || (o = -1), new si(o, this.dragging)
		}
		return this
	}
}
...
function jw(t, e, n) {
	...
	for (let c = 0; c < i.height; c++) {
		...
		const p = f.colwidth ? f.colwidth.slice() : Uw(f.colspan)
...
function rx(t, e, n = {}) {
	var r
	const o = (r = n.cellLineSeparator) != null ? r : tx
...
var ox = rx,
		...
		addProseMirrorPlugins() {
			return [
				...(this.options.resizable && this.editor.isEditable
					...
					: []),
				ah({ allowTableNodeSelection: this.options.allowTableNodeSelection }),
			]
		},
		extendNodeSchema(t) {
	...
	uh = j.create({
		...
		addExtensions() {
			const t = []
			return (
				this.options.table !== !1 && t.push(ix.configure(this.options.table)),
...
function wh(t, e, n, r) {
	let o = $e(t),
		i = px(Ne(t), n === "start", r)
	return (
		o && ((i = i.map((s) => s + "-" + o)), e && (i = i.concat(i.map(gr)))), i
	)
}
function yr(t) {
...
function fi(t) {
	return typeof t != "number" ? mx(t) : { top: t, right: t, bottom: t, left: t }
}
function xt(t) {
				...
				const k = w
						.map((A) => {
							...
							return [
								A.placement,
								z && d
									? A.overflows.slice(0, 2).reduce((V, U) => V + U, 0)
									: A.overflows[0],
								A.overflows,
							]
						})
					...
					O =
						((o = k.filter((A) =>
							A[2].slice(0, $e(A[0]) ? 2 : 3).every((z) => z <= 0),
						)[0]) == null
							? void 0
							: o[0]) || k[0][0]
				return O !== l
					? { data: { index: y + 1, overflows: w }, reset: { placement: O } }
					: {}
			},
		}
	),
				...
				if (
					...
					if (
						...
						(!(u === "alignment" ? b !== je(D) : !1) ||
							A.every((Y) => (je(Y.placement) === b ? Y.overflows[0] > 0 : !0)))
					)
						return { data: { index: _, overflows: A }, reset: { placement: D } }
					let K =
								...
								const I =
									(U = A.filter((Y) => {
										...
										.map((Y) => [
											...
											Y.overflows
												.filter((oe) => oe > 0)
												.reduce((oe, Pt) => oe + Pt, 0),
										])
										.sort((Y, oe) => Y[1] - oe[1])[0]) == null
										? void 0
										: U[0]
								I && (K = I)
								break
		...
		async fn(e) {
			...
			switch (r) {
				case "referenceHidden": {
					const i = await nn(e, { ...o, elementContext: "reference" }),
						s = kh(i, n.reference)
					return { data: { referenceHiddenOffsets: s, referenceHidden: Sh(s) } }
				}
				case "escaped": {
				...
				function p() {
					...
					if (u.length >= 2) {
						...
						const g = Ne(n) === "left",
							...
							b = He(...u.map((A) => A.left)),
							x = u.filter((A) => (g ? A.left === b : A.right === y)),
							v = x[0].top,
	...
	Rh = (t) => (
		t === void 0 && (t = {}),
		{
			...
			async fn(e) {
				...
				return {
					...m,
					data: { x: m.x - n, y: m.y - r, enabled: { [f]: i, [u]: s } },
				}
			},
		}
	)
	...
	t === void 0 && (t = {}),
	{
		...
		async fn(e) {
			...
			let b = m - d.top - d.bottom,
				...
				T = w
			if (
				((n = e.middlewareData.shift) != null && n.enabled.x && (T = x),
				(r = e.middlewareData.shift) != null && r.enabled.y && (k = b),
				C && !f)
			) {
				const A = pe(d.left, 0),
					z = pe(d.right, 0),
					...
					U = pe(d.bottom, 0)
				h
					? (T = p - 2 * (A !== 0 || z !== 0 ? A + z : pe(d.left, d.right)))
					: (k = m - 2 * (V !== 0 || U !== 0 ? V + U : pe(d.top, d.bottom)))
			}
			await a({ ...e, availableWidth: T, availableHeight: k })
			const O = await s.getDimensions(l.floating)
			return p !== O.width || m !== O.height ? { reset: { rects: !0 } } : {}
		},
	}
)
function pi() {
...
function ot(t) {
	var e
	return (e = (Ph(t) ? t.ownerDocument : t.document) || window.document) == null
		? void 0
		: e.documentElement
}
function Ph(t) {
...
function Ue(t) {
	return pi() ? t instanceof HTMLElement || t instanceof Te(t).HTMLElement : !1
}
function Ih(t) {
...
function kt(t) {
	...
	const e = t.assignedSlot || t.parentNode || (Ih(t) && t.host) || ot(t)
...
function Rx(t) {
	let e = ot(t),
		...
		l = -n.scrollTop
	return (
		Ve(r).direction === "rtl" && (s += pe(e.clientWidth, r.clientWidth) - o),
	...
	for (; Fe(s) && !on(s); ) {
		...
		!a && l.position === "fixed" && (o = null),
			(
				i
					? !a && !o
					: (!a && l.position === "static" && !!o && Ix.has(o.position)) ||
						(Nn(s) && !a && Uh(t, s))
			)
				? (r = r.filter((d) => d !== s))
				: (o = l),
...
var $x = async function (t) {
	const e = this.getOffsetParent || Kh,
...
var tp = async ({
	...
	const x = [
		...
		v = await Promise.all(
			r.map(async (w) => {
				...
				try {
					const k = (await import(w)).default
	...
	for (let w of v) {
		...
		const C = x.findIndex((k) => k.name === w.name)
		w.name === "placeholder" &&
			w.parent === null &&
...
var Wx = class {
		...
		get virtualElement() {
			...
			const { selection: e } = this.editor.state,
				n =
					(t = this.getReferencedVirtualElement) == null ? void 0 : t.call(this)
			if (n) return n
			let r = eu(this.view, e.from, e.to),
				o = { getBoundingClientRect: () => r, getClientRects: () => [r] }
			if (e instanceof L) {
			...
			if (e instanceof G) {
				...
				const u =
					c === d
						...
						: _x(c.getBoundingClientRect(), d.getBoundingClientRect())
	...
	sa = (t) =>
		new B({
			key: typeof t.pluginKey == "string" ? new $(t.pluginKey) : t.pluginKey,
...
function jx({
	...
	return {
		...
		async init() {
			...
			const z = "paragraph" in l
			Object.keys(l).forEach((I) => {
				const Y = this.$refs[`floatingToolbar::${I}`]
				if (!Y) {
				...
				T.registerPlugin(
					sa({
						...
						shouldShow: ({ editor: oe }) =>
							I === "paragraph"
								? oe.isFocused && oe.isActive(I) && !oe.state.selection.empty
								: z && !oe.state.selection.empty && oe.isActive("paragraph")
									? !1
		...
		setEditorSelection(z) {
			z &&
				...
				T.chain()
					.command(
						({ tr: V }) => (
							V.setSelection(P.fromJSON(T.state.doc, this.editorSelection)), !0
						),
					)
					.run())
		},
		...
		isPanelActive(z = null) {
			return z === null ? this.activePanel !== null : this.activePanel === z
		},
		insertMergeTag(z) {
			T.chain()
				.focus()
