...
var ni = (n) => (
	n === void 0 && (n = {}),
	{
		...
		async fn(t) {
			...
			if (
				((V = [...V, { placement: o, overflows: _ }]), !_.every((k) => k <= 0))
			) {
				var W, tt
				const k = (((W = s.flip) == null ? void 0 : W.index) || 0) + 1,
					$ = q[k]
				if (
					$ &&
					(!(d === "alignment" ? w !== ht($) : !1) ||
						V.every((L) => (ht(L.placement) === w ? L.overflows[0] > 0 : !0)))
				)
					return { data: { index: k, overflows: V }, reset: { placement: $ } }
				let B =
	...
	_onDragStart: function (t, e) {
		...
		G("setupClone", this),
			...
			(this.cloneId = me(() => {
				G("clone", this),
					!v.eventCanceled &&
						(this.options.removeCloneOnHide || I.insertBefore(T, h),
						this._hideClone(),
				...
				ft(
					{
						...
						target: i,
						completed: w,
						onMove: (Ge, Gi) => he(I, e, h, o, Ge, N(Ge), t, Gi),
						changed: D,
					},
					et,
				),
		...
		function w(L) {
			return (
				...
				L &&
					...
					g !== p &&
						(Q(h, H ? H.options.ghostClass : c.options.ghostClass, !1),
						Q(h, a.ghostClass, !0)),
					H !== g && g !== v.active
						? (H = g)
						: g === v.active && H && (H = null),
		...
		;(Mt = !1),
			...
			t &&
				...
				h &&
					...
					Yt &&
						!Mt &&
						Q(h, H ? H.options.ghostClass : this.options.ghostClass, !1),
					...
					I !== R
						? (Z >= 0 &&
								...
								Y({ sortable: this, name: "sort", toEl: R, originalEvent: t })),
							H && H.save())
						: Z !== Pt &&
							Z >= 0 &&
							(Y({ sortable: this, name: "update", toEl: R, originalEvent: t }),
							Y({ sortable: this, name: "sort", toEl: R, originalEvent: t })),
					v.active &&
						((Z == null || Z === -1) && ((Z = Pt), (wt = qt)),
						Y({ sortable: this, name: "end", toEl: R, originalEvent: t }),
						this.save()))),
			this._nulling()
	},
	_nulling: function () {
					...
					? ((C =
						...
						(F =
							...
							(_.overflowY === "auto" ||
								...
								_.overflowY === "visible")))
					: ((C =
							D < q && (_.overflowX === "auto" || _.overflowX === "scroll")),
						(F = A < J && (_.overflowY === "auto" || _.overflowY === "scroll")))
				var tt =
						C &&
						(Math.abs(w - o) <= r && V + D < q) - (Math.abs(O - o) <= r && !!V),
					z =
						F &&
						(Math.abs(S - s) <= r && W + A < J) - (Math.abs(m - s) <= r && !!W)
					...
					(tt != 0 || z != 0) &&
						...
						(P[d].pid = setInterval(
							function () {
								...
								var $ = P[this.layer].vy ? P[this.layer].vy * a : 0,
									B = P[this.layer].vx ? P[this.layer].vx * a : 0
								;(typeof f == "function" &&
									f.call(
