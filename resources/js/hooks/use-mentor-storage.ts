...
export function useMentorStorage(courses: MentorCourse[]) {
	...
	const [selectedCourse, setSelectedCourse] = useState<MentorCourse | null>(
		null,
	)
			...
			try {
				...
				if (savedState) {
					...
					if (parsedState.selectedCourseId) {
						const savedCourse = courses.find(
							(c) => c.id === parsedState.selectedCourseId,
						)
						if (savedCourse) {
							/*  console.log(
				...
				if (loadedCourse) {
					console.error(
						"Error loading state, using loaded course:",
						loadedCourse.id,
					)
					setSelectedCourse(loadedCourse)
				}
	...
	useEffect(() => {
		...
		if (updatedCourse) {
			...
			const hasNewData =
				updatedCourse.loaded !== selectedCourse.loaded ||
