...
export function SoloModal({
	...
	const [requirements, setRequirements] = useState<RequirementsStatus | null>(
		null,
	)
	...
	const renderMoodleStatus = () => {
		...
		const incompleteCourses =
			moodle.details?.filter((d) => !d.completed).length || 0
					...
					{mode === "remove" && (
						<div className="space-y-4">
							<Alert variant="destructive">
								<AlertCircle className="h-4 w-4" />
								<AlertDescription>
									Are you sure you want to remove this solo endorsement? This
									action cannot be undone. The trainee will immediately lose
									their solo privileges.
								</AlertDescription>
							</Alert>
