...
const getRatingColor = (rating: number): string => {
	switch (rating) {
		...
		default:
			return "bg-secondary-100 text-secondary-800 border-secondary-300 dark:bg-secondary-900/30 dark:text-secondary-400 dark:border-secondary-800"
	}
}
			...
			<div
				...
				onClick={(e) => {
					...
					if (target.tagName === "IMG") {
						...
						setPreviewImage({
							src: img.src,
							alt: img.alt || "Training log image",
						})
					}
				}}
				role="img"
...
export default function ViewTrainingLog({
	log,
	canEdit,
	canViewInternal,
	categories,
}: TrainingLogShowProps) {
	return (
				...
				<div className="flex flex-wrap items-start justify-between gap-4">
					<div>
						<h1 className="text-3xl font-bold">
							<span className="font-monospace">{log.position}</span> Training
							Session
						</h1>
						{log.course && (
						...
						<div>
							...
							<div className="grid gap-4 md:grid-cols-2">
								{categories.map((category) => {
									...
									return (
										<Card className="gap-0 overflow-hidden" key={category.name}>
												...
												{evaluation.positives || evaluation.negatives ? (
													<div className="space-y-4">
														...
														{evaluation.negatives && (
															<div>
																<h4 className="mb-2 flex items-center gap-2 text-sm font-medium text-warning-700 dark:text-warning-400">
																	<AlertCircle className="h-4 w-4" />
																	Areas for Improvement
																</h4>
																<MarkdownContent
																	content={evaluation.negatives}
																/>
															</div>
														)}
													</div>
