import { Head, router, useForm } from "@inertiajs/react"
import {
	CheckCircle2,
	ChevronDown,
	Info,
	InfoIcon,
	Loader2,
	Save,
	XCircle,
} from "lucide-react"
import { Fragment, useEffect, useState } from "react"
import { WYSIWYGEditor } from "@/components/logs/wysiwyg-editor"
import { Badge } from "@/components/ui/badge"
import { Button } from "@/components/ui/button"
import { Card, CardContent } from "@/components/ui/card"
import { Input } from "@/components/ui/input"
import { Label } from "@/components/ui/label"
import {
	Select,
	SelectContent,
	SelectItem,
	SelectTrigger,
	SelectValue,
} from "@/components/ui/select"
import { Separator } from "@/components/ui/separator"
import {
	Tooltip,
	TooltipContent,
	TooltipTrigger,
} from "@/components/ui/tooltip"
import { useDebounce } from "@/hooks/use-debounce"
import AppLayout from "@/layouts/app-layout"
import { cn } from "@/lib/utils"
import type { BreadcrumbItem } from "@/types"

interface EvaluationCategory {
	name: string
	label: string
	description: string
}

interface TrainingLogProps {
	log?: {
		id: number
		session_date: string
		position: string
		type: string
		traffic_level: string | null
		traffic_complexity: string | null
		runway_configuration: string | null
		surrounding_stations: string | null
		session_duration: number | null
		special_procedures: string | null
		airspace_restrictions: string | null
		evaluations?: {
			[key: string]: {
				rating: number
				rating_display: string
				positives: string | null
				negatives: string | null
			}
		}
		internal_remarks: string | null
		final_comment: string | null
		result: boolean
		next_step: string | null
	}
	trainee: {
		id: number
		name: string
		vatsim_id: number
	}
	course: {
		id: number
		name: string
		position: string
		type: string
	} | null
	categories: EvaluationCategory[]
	sessionTypes: { value: string; label: string }[]
	ratingOptions: { value: number; label: string }[]
	trafficLevels: { value: string; label: string }[]
	continueDraft?: boolean
	isEditing?: boolean
}

interface LogFormData {
	trainee_id?: number
	course_id?: number
	session_date: string
	position: string
	type: string
	traffic_level: string
	traffic_complexity: string
	runway_configuration: string
	surrounding_stations: string
	session_duration: string
	special_procedures: string
	airspace_restrictions: string
	theory: number
	theory_positives: string
	theory_negatives: string
	phraseology: number
	phraseology_positives: string
	phraseology_negatives: string
	coordination: number
	coordination_positives: string
	coordination_negatives: string
	tag_management: number
	tag_management_positives: string
	tag_management_negatives: string
	situational_awareness: number
	situational_awareness_positives: string
	situational_awareness_negatives: string
	problem_recognition: number
	problem_recognition_positives: string
	problem_recognition_negatives: string
	traffic_planning: number
	traffic_planning_positives: string
	traffic_planning_negatives: string
	reaction: number
	reaction_positives: string
	reaction_negatives: string
	separation: number
	separation_positives: string
	separation_negatives: string
	efficiency: number
	efficiency_positives: string
	efficiency_negatives: string
	ability_to_work_under_pressure: number
	ability_to_work_under_pressure_positives: string
	ability_to_work_under_pressure_negatives: string
	motivation: number
	motivation_positives: string
	motivation_negatives: string
	internal_remarks: string
	final_comment: string
	result: boolean | null
	next_step: string
}

const DRAFT_STORAGE_KEY_PREFIX_CREATE = "training-log-draft-"
const DRAFT_STORAGE_KEY_PREFIX_EDIT = "training-log-edit-draft-"

export default function CreateEditTrainingLog({
	log,
	trainee,
	course,
	categories,
	sessionTypes,
	ratingOptions,
	trafficLevels,
	continueDraft = false,
	isEditing = false,
}: TrainingLogProps) {
	const [showAdditionalDetails, setShowAdditionalDetails] = useState(false)
	const [autoSaveStatus, setAutoSaveStatus] = useState<
		"idle" | "saving" | "saved"
	>("idle")

	const storageKey = isEditing
		? `${DRAFT_STORAGE_KEY_PREFIX_EDIT}${log?.id}`
		: `${DRAFT_STORAGE_KEY_PREFIX_CREATE}${trainee.id}-${course?.id}`

	const breadcrumbs: BreadcrumbItem[] = [
		{
			title: "Training Logs",
			href: isEditing ? route("training-logs.index") : "",
		},
		{
			title: isEditing ? "Edit Log" : "Create Log",
			href: isEditing ? "" : route("courses.index"),
		},
	]

	const getInitialFormData = (): LogFormData => {
		if (isEditing && log) {
			const getRating = (category: string): number => {
				if (
					log.evaluations?.[category] &&
					typeof log.evaluations[category].rating === "number"
				) {
					return log.evaluations[category].rating
				}
				return 0
			}

			const getFeedback = (
				category: string,
				type: "positives" | "negatives",
			): string => {
				if (log.evaluations?.[category]) {
					return log.evaluations[category][type] || ""
				}
				return ""
			}

			return {
				session_date: log.session_date || "",
				position: log.position || "",
				type: log.type || "",
				traffic_level: log.traffic_level || "",
				traffic_complexity: log.traffic_complexity || "",
				runway_configuration: log.runway_configuration || "",
				surrounding_stations: log.surrounding_stations || "",
				session_duration:
					log.session_duration !== null ? log.session_duration.toString() : "",
				special_procedures: log.special_procedures || "",
				airspace_restrictions: log.airspace_restrictions || "",

				theory: getRating("theory"),
				theory_positives: getFeedback("theory", "positives"),
				theory_negatives: getFeedback("theory", "negatives"),

				phraseology: getRating("phraseology"),
				phraseology_positives: getFeedback("phraseology", "positives"),
				phraseology_negatives: getFeedback("phraseology", "negatives"),

				coordination: getRating("coordination"),
				coordination_positives: getFeedback("coordination", "positives"),
				coordination_negatives: getFeedback("coordination", "negatives"),

				tag_management: getRating("tag_management"),
				tag_management_positives: getFeedback("tag_management", "positives"),
				tag_management_negatives: getFeedback("tag_management", "negatives"),

				situational_awareness: getRating("situational_awareness"),
				situational_awareness_positives: getFeedback(
					"situational_awareness",
					"positives",
				),
				situational_awareness_negatives: getFeedback(
					"situational_awareness",
					"negatives",
				),

				problem_recognition: getRating("problem_recognition"),
				problem_recognition_positives: getFeedback(
					"problem_recognition",
					"positives",
				),
				problem_recognition_negatives: getFeedback(
					"problem_recognition",
					"negatives",
				),

				traffic_planning: getRating("traffic_planning"),
				traffic_planning_positives: getFeedback(
					"traffic_planning",
					"positives",
				),
				traffic_planning_negatives: getFeedback(
					"traffic_planning",
					"negatives",
				),

				reaction: getRating("reaction"),
				reaction_positives: getFeedback("reaction", "positives"),
				reaction_negatives: getFeedback("reaction", "negatives"),

				separation: getRating("separation"),
				separation_positives: getFeedback("separation", "positives"),
				separation_negatives: getFeedback("separation", "negatives"),

				efficiency: getRating("efficiency"),
				efficiency_positives: getFeedback("efficiency", "positives"),
				efficiency_negatives: getFeedback("efficiency", "negatives"),

				ability_to_work_under_pressure: getRating(
					"ability_to_work_under_pressure",
				),
				ability_to_work_under_pressure_positives: getFeedback(
					"ability_to_work_under_pressure",
					"positives",
				),
				ability_to_work_under_pressure_negatives: getFeedback(
					"ability_to_work_under_pressure",
					"negatives",
				),

				motivation: getRating("motivation"),
				motivation_positives: getFeedback("motivation", "positives"),
				motivation_negatives: getFeedback("motivation", "negatives"),

				internal_remarks: log.internal_remarks || "",
				final_comment: log.final_comment || "",
				result: log.result,
				next_step: log.next_step || "",
			}
		} else {
			return {
				trainee_id: trainee.id,
				course_id: course?.id,
				session_date: new Date().toISOString().split("T")[0],
				position: "",
				type: "",
				traffic_level: "",
				traffic_complexity: "",
				runway_configuration: "",
				surrounding_stations: "",
				session_duration: "",
				special_procedures: "",
				airspace_restrictions: "",
				theory: 0,
				theory_positives: "",
				theory_negatives: "",
				phraseology: 0,
				phraseology_positives: "",
				phraseology_negatives: "",
				coordination: 0,
				coordination_positives: "",
				coordination_negatives: "",
				tag_management: 0,
				tag_management_positives: "",
				tag_management_negatives: "",
				situational_awareness: 0,
				situational_awareness_positives: "",
				situational_awareness_negatives: "",
				problem_recognition: 0,
				problem_recognition_positives: "",
				problem_recognition_negatives: "",
				traffic_planning: 0,
				traffic_planning_positives: "",
				traffic_planning_negatives: "",
				reaction: 0,
				reaction_positives: "",
				reaction_negatives: "",
				separation: 0,
				separation_positives: "",
				separation_negatives: "",
				efficiency: 0,
				efficiency_positives: "",
				efficiency_negatives: "",
				ability_to_work_under_pressure: 0,
				ability_to_work_under_pressure_positives: "",
				ability_to_work_under_pressure_negatives: "",
				motivation: 0,
				motivation_positives: "",
				motivation_negatives: "",
				internal_remarks: "",
				final_comment: "",
				result: null,
				next_step: "",
			}
		}
	}

	const { data, setData, post, put, processing, errors } = useForm<LogFormData>(
		getInitialFormData(),
	)

	useEffect(() => {
		if (!continueDraft) {
			if (!isEditing) {
				localStorage.removeItem(storageKey)
			}
			return
		}

		const savedDraft = localStorage.getItem(storageKey)
		if (savedDraft) {
			try {
				const parsedDraft = JSON.parse(savedDraft)
				setData(parsedDraft)
				/* console.log(`${isEditing ? 'Edit' : 'Create'} mode - Loaded draft from localStorage`); */
			} catch (error) {
				console.error("Failed to load draft:", error)
			}
		}
	}, [continueDraft, isEditing, setData, storageKey])

	useEffect(() => {
		const hasAdditionalDetails =
			data.traffic_level ||
			data.traffic_complexity ||
			data.runway_configuration ||
			data.surrounding_stations ||
			data.special_procedures ||
			data.airspace_restrictions

		if (hasAdditionalDetails) {
			setShowAdditionalDetails(true)
		}
	}, [
		data.airspace_restrictions,
		data.runway_configuration,
		data.special_procedures,
		data.surrounding_stations,
		data.traffic_complexity,
		data.traffic_level,
	])

	const debouncedData = useDebounce(data, 1000)

	useEffect(() => {
		if (debouncedData) {
			setAutoSaveStatus("saving")
			try {
				localStorage.setItem(storageKey, JSON.stringify(debouncedData))
				setAutoSaveStatus("saved")
				setTimeout(() => setAutoSaveStatus("idle"), 2000)
			} catch (error) {
				console.error("Failed to save draft:", error)
			}
		}
	}, [debouncedData, storageKey])

	const hasErrors = Object.keys(errors).length > 0

	const handleSubmit = (e: React.FormEvent) => {
		e.preventDefault()

		if (isEditing && log) {
			put(route("training-logs.update", log.id), {
				onSuccess: () => localStorage.removeItem(storageKey),
			})
		} else {
			post(route("training-logs.store"), {
				onSuccess: () => localStorage.removeItem(storageKey),
			})
		}
	}

	const handleCancel = () => {
		if (isEditing && log) {
			router.visit(route("training-logs.show", log.id))
		} else {
			router.visit(route("overview.index"))
		}
	}

	const RatingButton = ({
		value,
		label,
		selected,
		onClick,
	}: {
		value: number
		label: string
		selected: boolean
		onClick: () => void
	}) => {
		const getColorClasses = () => {
			if (!selected) return ""

			switch (value) {
				case 4:
					return "border-2 border-success-600 bg-success-600 text-success-foreground hover:bg-success-700 hover:text-success-foreground"
				case 3:
					return "border-2 border-primary-600 bg-primary-600 text-primary-foreground hover:bg-primary-700 hover:text-primary-foreground"
				case 2:
					return "border-2 border-warning-700 bg-warning-700 text-warning-foreground hover:bg-warning-800 hover:text-warning-foreground"
				case 1:
					return "border-2 border-danger-600 bg-danger-600 text-destructive-foreground hover:bg-danger-700 hover:text-destructive-foreground"
				default:
					return "border-2 dark:border-primary"
			}
		}

		return (
			<Tooltip delayDuration={200}>
				<TooltipTrigger asChild>
					<Button
						className={cn(
							"flex size-8 items-center justify-center rounded-lg text-base font-bold transition-all duration-200",
							getColorClasses(),
						)}
						onClick={onClick}
						type="button"
						variant={"outline"}
					>
						{value === 0 ? "—" : value}
					</Button>
				</TooltipTrigger>
				<TooltipContent>
					<p>{label}</p>
				</TooltipContent>
			</Tooltip>
		)
	}

	return (
		<AppLayout breadcrumbs={breadcrumbs}>
			<Head
				title={`${isEditing ? "Edit" : "Create"} Training Log - ${trainee.name}`}
			/>

			<div className="flex h-full flex-1 flex-col gap-6 overflow-x-auto rounded-xl p-4">
				<form className="space-y-6" onSubmit={handleSubmit}>
					{hasErrors && (
						<div className="flex items-start gap-3 rounded-lg border border-danger-200 bg-danger-50 p-4 dark:border-danger-800 dark:bg-danger-950">
							<XCircle className="mt-0.5 h-5 w-5 shrink-0 text-danger-500" />
							<div>
								<p className="font-medium text-danger-800 dark:text-danger-200">
									Please fix the following errors before submitting
								</p>
								<ul className="mt-1 list-disc pl-4 text-sm text-danger-700 dark:text-danger-300">
									{Object.entries(errors).map(([field, message]) => (
										<li key={field}>{message as string}</li>
									))}
								</ul>
							</div>
						</div>
					)}
					{/* Basic Session Information */}
					<Card>
						<CardContent className="gap-0">
							<div className="mb-6 flex items-center justify-between">
								<div>
									<h2 className="text-xl font-semibold">
										{isEditing
											? "Edit Session Information"
											: "Session Information"}
									</h2>
									<p className="mt-1 flex gap-1 text-sm text-muted-foreground">
										{trainee.name} •{" "}
										{course && <Badge variant={"outline"}>{course.name}</Badge>}
									</p>
								</div>
								<div className="flex items-center gap-2">
									{isEditing && log && (
										<Badge variant="secondary">Editing Log #{log.id}</Badge>
									)}
									{autoSaveStatus !== "idle" && (
										<Badge
											variant={
												autoSaveStatus === "saved" ? "default" : "secondary"
											}
										>
											{autoSaveStatus === "saving" ? (
												<>
													<Loader2 className="mr-1 h-3 w-3 animate-spin" />
													Saving...
												</>
											) : (
												<>
													<CheckCircle2 className="mr-1 h-3 w-3" />
													Saved
												</>
											)}
										</Badge>
									)}
								</div>
							</div>

							<div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
								<div className="space-y-2">
									<Label htmlFor="session_date">Date *</Label>
									<Input
										className={errors.session_date ? "border-destructive" : ""}
										id="session_date"
										max={new Date().toISOString().split("T")[0]}
										onChange={(e) => setData("session_date", e.target.value)}
										type="date"
										value={data.session_date}
									/>
									{errors.session_date && (
										<p className="text-sm text-destructive">
											{errors.session_date}
										</p>
									)}
								</div>

								<div className="space-y-2">
									<Label htmlFor="position">Position *</Label>
									<Input
										className={errors.position ? "border-destructive" : ""}
										id="position"
										maxLength={25}
										onChange={(e) => setData("position", e.target.value)}
										placeholder="e.g., EDDF_N_APP"
										value={data.position}
									/>
									{errors.position && (
										<p className="text-sm text-destructive">
											{errors.position}
										</p>
									)}
								</div>

								<div className="space-y-2">
									<Label htmlFor="session_duration">Duration (minutes)</Label>
									<Input
										className={
											errors.session_duration ? "border-destructive" : ""
										}
										id="session_duration"
										max="480"
										min="1"
										onChange={(e) =>
											setData("session_duration", e.target.value)
										}
										placeholder="90"
										type="number"
										value={data.session_duration}
									/>
									{errors.session_duration && (
										<p className="text-sm text-destructive">
											{errors.session_duration}
										</p>
									)}
								</div>

								<div className="space-y-2">
									<Label htmlFor="type">Session Type *</Label>
									<Select
										onValueChange={(value) => setData("type", value)}
										value={data.type}
									>
										<SelectTrigger
											className={errors.type ? "border-destructive" : ""}
											id="type"
										>
											<SelectValue placeholder="Select type..." />
										</SelectTrigger>
										<SelectContent>
											{sessionTypes.map((type) => (
												<SelectItem key={type.value} value={type.value}>
													{type.label}
												</SelectItem>
											))}
										</SelectContent>
									</Select>
									{errors.type && (
										<p className="text-sm text-destructive">{errors.type}</p>
									)}
								</div>
							</div>

							{/* Additional Details - Collapsible */}
							<div className="mt-6">
								<button
									className="flex w-full items-center justify-between rounded-lg border bg-muted/30 p-4 text-left transition-colors hover:bg-muted/50"
									onClick={() =>
										setShowAdditionalDetails(!showAdditionalDetails)
									}
									type="button"
								>
									<span className="font-medium">
										Additional Session Details
									</span>
									<ChevronDown
										className={cn(
											"h-5 w-5 transition-transform duration-200",
											showAdditionalDetails && "rotate-180",
										)}
									/>
								</button>

								{showAdditionalDetails && (
									<div className="mt-4 space-y-4 rounded-lg border bg-muted/10 p-4">
										<div className="grid gap-4 md:grid-cols-2">
											<div className="space-y-2">
												<Label htmlFor="traffic_level">Traffic Level</Label>
												<Select
													onValueChange={(value) =>
														setData(
															"traffic_level",
															value === "none" ? "" : value,
														)
													}
													value={data.traffic_level || "none"}
												>
													<SelectTrigger id="traffic_level">
														<SelectValue placeholder="Not specified" />
													</SelectTrigger>
													<SelectContent>
														<SelectItem value="none">Not specified</SelectItem>
														{trafficLevels.map((level) => (
															<SelectItem key={level.value} value={level.value}>
																{level.label}
															</SelectItem>
														))}
													</SelectContent>
												</Select>
											</div>

											<div className="space-y-2">
												<Label htmlFor="traffic_complexity">
													Traffic Complexity
												</Label>
												<Select
													onValueChange={(value) =>
														setData(
															"traffic_complexity",
															value === "none" ? "" : value,
														)
													}
													value={data.traffic_complexity || "none"}
												>
													<SelectTrigger id="traffic_complexity">
														<SelectValue placeholder="Not specified" />
													</SelectTrigger>
													<SelectContent>
														<SelectItem value="none">Not specified</SelectItem>
														{trafficLevels.map((level) => (
															<SelectItem key={level.value} value={level.value}>
																{level.label}
															</SelectItem>
														))}
													</SelectContent>
												</Select>
											</div>

											<div className="space-y-2">
												<Label htmlFor="runway_configuration">
													Runway Configuration
												</Label>
												<Input
													className={
														errors.runway_configuration
															? "border-destructive"
															: ""
													}
													id="runway_configuration"
													maxLength={50}
													onChange={(e) =>
														setData("runway_configuration", e.target.value)
													}
													placeholder="e.g., 25L/07R"
													value={data.runway_configuration}
												/>
												{errors.runway_configuration && (
													<p className="text-sm text-destructive">
														{errors.runway_configuration}
													</p>
												)}
											</div>

											<div className="space-y-2">
												<Label htmlFor="surrounding_stations">
													Surrounding Stations
												</Label>
												<Input
													className={
														errors.surrounding_stations
															? "border-destructive"
															: ""
													}
													id="surrounding_stations"
													onChange={(e) =>
														setData("surrounding_stations", e.target.value)
													}
													placeholder="e.g., EDDF_C_TWR, EDDF_C_GND"
													value={data.surrounding_stations}
												/>
												{errors.surrounding_stations && (
													<p className="text-sm text-destructive">
														{errors.surrounding_stations}
													</p>
												)}
											</div>
										</div>

										<div className="space-y-2">
											<Label htmlFor="special_procedures">
												Special Procedures
											</Label>
											<WYSIWYGEditor
												minHeight="120px"
												onChange={(value) =>
													setData("special_procedures", value)
												}
												placeholder="Describe any special procedures used..."
												value={data.special_procedures}
											/>
											{errors.special_procedures && (
												<p className="text-sm text-destructive">
													{errors.special_procedures}
												</p>
											)}
										</div>

										<div className="space-y-2">
											<Label htmlFor="airspace_restrictions">
												Airspace Restrictions
											</Label>
											<WYSIWYGEditor
												minHeight="120px"
												onChange={(value) =>
													setData("airspace_restrictions", value)
												}
												placeholder="Note any airspace restrictions..."
												value={data.airspace_restrictions}
											/>
											{errors.airspace_restrictions && (
												<p className="text-sm text-destructive">
													{errors.airspace_restrictions}
												</p>
											)}
										</div>
									</div>
								)}
							</div>
						</CardContent>
					</Card>

					{/* Performance Evaluation */}
					<div className="space-y-4">
						<h2 className="text-2xl font-bold">Performance Evaluation</h2>

						<div className="space-y-4">
							<Card className="overflow-hidden">
								<CardContent className="space-y-8">
									{categories.map((category, index) => (
										<Fragment key={category.name}>
											<div className="mb-6 flex items-center justify-between">
												<div className="flex items-center justify-center gap-3">
													<div className="flex h-10 w-10 items-center justify-center rounded-lg bg-primary/10 text-lg font-bold text-primary dark:bg-primary/20">
														{index + 1}
													</div>
													<h3 className="text-center text-lg font-semibold">
														{category.label}
													</h3>
													<Tooltip>
														<TooltipTrigger asChild>
															<InfoIcon className="size-3 text-accent-500" />
														</TooltipTrigger>
														<TooltipContent>
															<p>{category.description}</p>
														</TooltipContent>
													</Tooltip>
												</div>
												<div className="flex gap-2">
													{ratingOptions.map((option) => (
														<RatingButton
															key={option.value}
															label={option.label}
															onClick={() =>
																setData(
																	category.name as keyof LogFormData,
																	option.value,
																)
															}
															selected={
																data[category.name as keyof LogFormData] ===
																option.value
															}
															value={option.value}
														/>
													))}
												</div>
											</div>

											<div className="grid gap-4 md:grid-cols-2">
												<div className="space-y-2">
													<Label className="flex items-center gap-2 text-base text-success">
														<CheckCircle2 className="h-4 w-4" />
														Strengths
													</Label>
													<WYSIWYGEditor
														minHeight="150px"
														onChange={(value) =>
															setData(
																`${category.name}_positives` as keyof LogFormData,
																value,
															)
														}
														placeholder=""
														value={
															data[
																`${category.name}_positives` as keyof LogFormData
															] as string
														}
													/>
													{errors[
														`${category.name}_positives` as keyof typeof errors
													] && (
														<p className="text-sm text-destructive">
															{
																errors[
																	`${category.name}_positives` as keyof typeof errors
																]
															}
														</p>
													)}
												</div>

												<div className="space-y-2">
													<Label className="flex items-center gap-2 text-base text-warning-800 dark:text-warning-600">
														<Info className="h-4 w-4" />
														Areas for Improvement
													</Label>
													<WYSIWYGEditor
														minHeight="150px"
														onChange={(value) =>
															setData(
																`${category.name}_negatives` as keyof LogFormData,
																value,
															)
														}
														placeholder=""
														value={
															data[
																`${category.name}_negatives` as keyof LogFormData
															] as string
														}
													/>
													{errors[
														`${category.name}_negatives` as keyof typeof errors
													] && (
														<p className="text-sm text-destructive">
															{
																errors[
																	`${category.name}_negatives` as keyof typeof errors
																]
															}
														</p>
													)}
												</div>
											</div>
										</Fragment>
									))}
								</CardContent>
							</Card>
						</div>
					</div>

					{/* Final Assessment */}
					<Card className="border-2">
						<CardContent className="gap-0">
							<h2 className="mb-6 text-2xl font-bold">Final Assessment</h2>

							<div className="space-y-6">
								<div className="space-y-2">
									<Label className="text-base" htmlFor="final_comment">
										Overall Session Summary
									</Label>
									<WYSIWYGEditor
										minHeight="200px"
										onChange={(value) => setData("final_comment", value)}
										placeholder="Provide a comprehensive assessment of the trainee's overall performance during this session..."
										value={data.final_comment}
									/>
									{errors.final_comment && (
										<p className="text-sm text-destructive">
											{errors.final_comment}
										</p>
									)}
								</div>

								<div className="space-y-2">
									<Label className="text-base" htmlFor="next_step">
										Next Training Step
									</Label>
									<Input
										className={errors.next_step ? "border-destructive" : ""}
										id="next_step"
										onChange={(e) => setData("next_step", e.target.value)}
										placeholder="e.g., Continue with complex approach scenarios"
										value={data.next_step}
									/>
									{errors.next_step && (
										<p className="text-sm text-destructive">
											{errors.next_step}
										</p>
									)}
								</div>

								<Separator />

								<div className="space-y-2">
									<Label
										className="flex items-center gap-2 text-base text-muted-foreground"
										htmlFor="internal_remarks"
									>
										Internal Mentor Notes
										<Badge className="text-xs" variant="secondary">
											Private
										</Badge>
									</Label>
									<WYSIWYGEditor
										minHeight="150px"
										onChange={(value) => setData("internal_remarks", value)}
										placeholder="Private notes for mentors only (not visible to trainee)..."
										value={data.internal_remarks}
									/>
									{errors.internal_remarks && (
										<p className="text-sm text-destructive">
											{errors.internal_remarks}
										</p>
									)}
								</div>

								<Separator />

								{/* Session Result */}
								<div className="flex flex-col space-y-3">
									<Label className="text-lg font-semibold">
										Session Result *
									</Label>
									<div className="grid grid-cols-2 gap-4">
										<Button
											className="py-8"
											onClick={() => setData("result", true)}
											type="button"
											variant={data.result ? "success" : "outline"}
										>
											<CheckCircle2
												className={cn(
													"h-12 w-12 transition-colors",
													data.result === true
														? "text-success"
														: "text-muted-foreground group-hover:text-success",
												)}
											/>
											<span
												className={cn(
													"text-lg font-bold transition-colors",
													data.result === true
														? "text-success"
														: "text-muted-foreground group-hover:text-success",
												)}
											>
												Passed
											</span>
										</Button>

										<Button
											className="py-8"
											onClick={() => setData("result", false)}
											type="button"
											variant={!data.result ? "destructive" : "outline"}
										>
											<XCircle className="h-12 w-12 transition-colors" />
											<span className="text-lg font-bold transition-colors">
												Not Passed
											</span>
										</Button>
									</div>
									{errors.result && (
										<p className="text-sm text-destructive">{errors.result}</p>
									)}
								</div>
							</div>
						</CardContent>
					</Card>

					{/* Action Buttons */}
					<div className="flex items-center justify-between rounded-xl border-2 bg-muted/30 p-6">
						<Button
							onClick={handleCancel}
							size="lg"
							type="button"
							variant="outline"
						>
							Cancel
						</Button>
						<Button
							className="min-w-[200px]"
							disabled={processing}
							size="lg"
							type="submit"
						>
							{processing ? (
								<>
									<Loader2 className="mr-2 h-5 w-5 animate-spin" />
									{isEditing ? "Saving Changes..." : "Submitting..."}
								</>
							) : (
								<>
									{isEditing ? (
										<Save className="mr-2 h-5 w-5" />
									) : (
										<CheckCircle2 className="mr-2 h-5 w-5" />
									)}
									{isEditing ? "Save Changes" : "Submit Training Log"}
								</>
							)}
						</Button>
					</div>
				</form>
			</div>
		</AppLayout>
	)
}
