...
export default function CptUpload({
	...
	const handleGrade = (passed: boolean) => {
		if (
			confirm(
				`Are you sure you want to mark this CPT as ${passed ? "passed" : "failed"}?`,
			)
		) {
			router.post(
				route("cpt.grade", { cpt: cpt.id, result: passed ? 1 : 0 }),
				{},
				{
					preserveScroll: true,
										...
										<div
											className={`flex cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed p-12 transition-colors ${
												dragActive
													...
													: "border-primary-300 bg-primary-50 hover:bg-primary-100 dark:border-primary-700 dark:bg-primary-900/20"
											}`}
											onClick={() =>
											...
											role="document"
										>
											<Upload className="mb-4 h-16 w-16 text-primary-600" />
											<h3 className="mb-2 text-lg font-medium text-primary-600">
												Upload CPT Log
											</h3>
											...
											<div className="rounded-lg border border-success-500 bg-success-50 p-4 dark:bg-success-900/20">
												<div className="flex items-center justify-between">
													<div className="flex items-center gap-3">
														<div className="rounded bg-danger-100 p-2 dark:bg-danger-900">
															<Eye className="h-6 w-6 text-danger-600" />
														</div>
														<div>
						...
						{logs.length > 0 && (
							<Card>
								<CardHeader>
									...
									<p className="text-sm text-muted-foreground">
										{logs.length} document{logs.length !== 1 && "s"} uploaded
									</p>
								</CardHeader>
										...
										<div className="p-6" key={log.id}>
											<div className="flex items-center justify-between">
												<div className="flex items-center gap-4">
													<div className="rounded bg-danger-100 p-3 dark:bg-danger-900">
														<File className="h-6 w-6 text-danger-600" />
													</div>
													<div>
						...
						{logs.length > 0 && logs[0] && (
							<Card>
								<CardHeader>
									...
									<p className="text-sm text-muted-foreground">
										Preview of the uploaded training log
									</p>
								</CardHeader>
								<CardContent>
